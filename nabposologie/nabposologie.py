...
class Nabposologie(NabService):
    DAEMON_PIDFILE = "/run/nabposologie.pid"
    ...
    async def setup_listener(self):
        ...
        if config.enabled:
            packet = (
                ""
                '{"type":"mode","mode":"idle",'
                '"events":["button","asr/nabposologie"],'
                '"request_id":"idle-button"}\r\n'
            )
        else:
            packet = (
                '{"type":"mode","mode":"idle","events":["asr/nabposologie"],'
    ...
    async def perform(self):
        packet = (
            '{"type":"message",'
            '"body":[{"audio":["nabposologie/answers/*.mp3"]}],'
    ...
    async def entered_interactive(self):
        ...
        resp = (
            '{"type":"command",'
            '"sequence":[{"audio":["nabposologie/listen.mp3"],'
    ...
    async def exit_interactive(self):
        packet = (
            '{"type":"command",'
            '"sequence":[{"audio":["nabposologie/acquired.mp3"]}],'
    ...
    async def process_asr_event_packet(self, packet):
        if packet["nlu"]["intent"] == "nabposologie/8ball":
            await self.perform()
